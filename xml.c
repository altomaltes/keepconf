 ...
 */
     ...
     //       puts("");
      //    break;
          ...
          case  ',':         /* Test for array */
            if ( xml->ptr[ -1 ] != '\\' )  // 2020, escape string
            { pushElement( xml->data, xml->idx ? xml->idx : -xml->items
                         , xml->buffItm
                         , xml->buffNme
                         , xml->buffVal );
          ...
          default   :
            if ( xml->ptr[ -1 ] == '\\' )  // 2020, was escaped
            { xml->ptr--;
            }
            *xml->ptr++= itm; break;  /* Add to serial                     */
        }
...
static char * getName( char * buff, const char * name, const char * path )
{ strcpy( buff, path );
  ...
  if ( !mk )
  { mk= strrchr( buff, '\\' );  // try mchoff
  }

  if ( mk )                    // Point to free space
  { buff= mk+1;
  }

  strcpy( buff, name ); strcat( buff, ".xml" );
...
{ char * fname= alloca( strlen( name ) + 1024 );
  ...
  getName( fname, name, path ); if (( xml->file= fopen( fname, "wt" ) ))
  { fprintf( xml->file
           , "<?xml version=\"1.0\" ?>\n"
             "<config style=\"%s\">\n"
           , "rich" );
