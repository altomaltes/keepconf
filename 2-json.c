...
ANSIC int jsonParse( KeeperRec * jsn, const char * dat, int sz  )
{ while( sz-- )
  { char itm= *dat++;
 ...
 */
      ...
      { case '[':                          /* Catch an array */
        ...
        case '{':                          /* Enter object */
          ...
          if ( jsn->array == INTEGER_NAM ) /* Compromised solution. Non array objects can omit typename */
          { pushElement( jsn->data, INTEGER_NAM
                       , jsn->buffNme
      ...
      { case ']':
        ...
        case ',': case '}':
           ...
           { *jsn->ptr=  0;
             ...
             { pushElement( jsn->data, 0           /* Pending value           */
                          , jsn->buffNme
                          , ""                     /* No type on leaves       */
                          , jsn->buffVal );        /* Stamp value             */
             }
...
{ int loop; int brck= ' '; int bunch;
  ...
  { if ( !*jsn->ptr )
    { fprintf( jsn->file, jsn->idx ? "  }\n"  : "  } ]\n" );
      ...
      if ( !jsn->level && jsn->idx <= 0 )  // 2019  EOF
      { fprintf( jsn->file, "}\n" );
      }
...
ANSIC KeeperRec  * jsonInit( KeeperRec  * jsn
                           , void       * autom )
{ jsn->data = autom;
  jsn->phase= PHASE_IDLE;
  jsn->ptr  = NULL;
