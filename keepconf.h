...
ANSIC const char * stringById( const char * str );
ANSIC const void * structById( const void * data, size_t sz );
...
/* Some implementations xlate strcasecmp to _stricmp, so the mess in windows
 */

  extern "C" int strcasecmp1( const char *s1, const char *s2);
...
{ int          index;
  ...
  { if ( names )
    { if ( strcasecmp1( name, names ))
      { return( ITEM_NAMES );   /* Not this object, try next  */
    } }

    if ( objs )
    { if ( *objs )
      { if ( strcasecmp1(  type, objs  ))
 ...
 */
  ...
  { struct HolderRec
    ...
    T          * ptr= nextObject( streamer, NULL );
    HolderRec  * list= NULL;
    int loop= 0;
  ...
  { const char * id= typeId( *t );
    ...
    { case OBJECT_SAVE:
        ...
        { if ( z )
          ...
          { if ( y )
            ...
            { if ( x ) { *ptr++= ','; }
              ...
              char * res= (char*)asString( ptr, *itm ); // Serialize

              if ( buffsz )                // Must be escaped
              { char * in=  res;
                char * dst= buff + buffsz; // Encode in reverse

                while( *in )
                { switch ( *in  )   // Must be escaped
                  { case ','   :
                    case '\\' :
                      *dst-- = '\\';     // Escape char
                  }
                  *dst--= *in++;      // Put one char in hole
                }
                *dst= 0;              // Terminator

                in=  res;
                dst= buff + buffsz; // Encode in reverse

                while( *dst )      // Copy zero also
                { *in++ = *dst--;
                }
                *in= 0;
              }
      ...
      case OBJECT_LOAD:        //  LOAD version
        ...
        { if ( buffsz )                // Told new object
          { itm[ info->index ]= (T)calloc( buffsz, 1 );
          }
 ...
 */
  ...
  ITEM_DONE launch(           long (&t), const char * id ) { return( launchStd( t, id )); }
  ITEM_DONE launch( unsigned  long (&t), const char * id ) { return( launchStd( t, id )); }
 ...
 */
  template < class T >
  int estimateXmlStreamBuffLen( const T & item )
  { return( 0  );                                 // This tells not to escape
  }
  int estimateXmlStreamBuffLen( char * ptr )      // String spetialization
  { int sz= 4;                                    // Add separators

    while( *ptr )
    { switch( *ptr )
      { case ',': case '\\': sz += 2; break;
        default :            sz += 1; break;
      }
      ptr++;
    }
  ...
  template < class T >
  ITEM_DONE launch( T ** &t, const char * id )
  { T ** ptr= t;
    int buffsz= 0;
    int sz= 0;
    ...
    { case OBJECT_SAVE:
        while( *ptr )               // Calculate
        { buffsz += estimateXmlStreamBuffLen( *ptr );
          ptr++; sz++;
        }
      break;
      ...
      case OBJECT_LOAD:
        sz= info->size;
        if ( !info->index )     // told create object
        { t= (T**)calloc( (sz + 1 ), sizeof(T) );
        }
        buffsz= strlen( value ) + 1;
      break;
    }


    return( launchStd( t, id, buffsz, sz ));
  }

/* For sized vectors    ççç
 */
...
{ void startLevels( T           & varObj
  ...
  void save( KeepSvr      svr
           , const char * varName, const char * path
           , T          & varObj )
...
{ KeepSvr      saver;
  ...
  ObjKeeper( T          & vObj          // Load object
           , KeepLdr      loader
           , const char * vName, const char * vPath= NULL  )
           : ObjStorer<T>( vObj, vName )
           , varObj(  vObj  )
           , varName( vName )
           , varPath( vPath )
  { saver= loader( varName, varPath
                 , this );  // Tell stub
  }

  ~ObjKeeper(  )                          // Save object on exit
  { ObjStorer<T>::save( saver, varName, varPath, varObj );
  }
};
...
#define KEEPXML( var, path ) ObjKeeper<typeof(var)>XMLKEEP##var( var, xmlLoad, #var, path )
#define KEEPJSN( var, path ) ObjKeeper<typeof(var)>XMLKEEP##var( var, jsnLoad, #var, path )
#define CAP_TYPEOF 1
